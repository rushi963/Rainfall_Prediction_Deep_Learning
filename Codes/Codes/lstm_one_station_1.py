...
with open(os.path.join(dataPath, "one_station_1.csv"), 'rt') as file:
    reader = csv.reader(file)
    X = []
    Y = []
    content = []
    #actual_length = 517
    Data = [[[0 for i in range(8)] for j in range(24)] for k in range(92)]
    data = np.array(Data, dtype=float)
    Target = [[0 for i in range(24)] for j in range(92)]
    ...
    #components = list(map(list, zip(*components)))
